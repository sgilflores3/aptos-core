...
use aptos_protos::transaction::v1::{UserTransaction as ProtoUserTransaction,
    TransactionPayload,
    transaction_payload::Payload as ProtoPayloadEnum};
...
impl UserTransaction {
    pub fn from_transaction(
        ...
        let user_request = txn.request.as_ref().expect("Sends is not present in user txn");
        (
            Self {
                version,
                block_height,
                ...
                expiration_timestamp_secs: parse_timestamp(
                    user_request.expiration_timestamp_secs.expect(
                    "Expiration timestamp is not present in user txn").seconds.try_into().unwrap(),
                    version),
                ...
                entry_function_id_str: match &user_request.payload.unwrap().payload{
                    Some(ProtoPayloadEnum::EntryFunctionPayload(payload)) => {
                        payload.function.expect("function not exists.").name
                    },
                    _ => String::default(),
                },
                epoch,
