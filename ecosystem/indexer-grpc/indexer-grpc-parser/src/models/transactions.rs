...
use aptos_protos::transaction::v1::{Transaction as TransactionProto, TransactionInfo, transaction::TxnData, transaction::TransactionType};

use bigdecimal::BigDecimal;
use diesel::{
    BelongingToDsl, ExpressionMethods, GroupedBy, OptionalExtension, QueryDsl, RunQueryDsl, IntoSql,
};
...
impl Transaction {
    fn from_transaction_info(
        ...
        Self {
            ...
            version,
            block_height,
            hash: String::from_utf8(info.hash).expect("Getting transaction hash failed."),
            state_change_hash: String::from_utf8(info.state_change_hash).expect("Getting state change hash failed."),
            event_root_hash: String::from_utf8(info.event_root_hash).expect("Getting event root hash failed."),
            state_checkpoint_hash: info.state_checkpoint_hash.map(|h| String::from_utf8(h).expect("Getting state checkpoint hash failed.")),
            gas_used: u64_to_bigdecimal(info.gas_used),
            success: info.success,
            vm_status: info.vm_status.clone(),
            accumulator_root_hash: String::from_utf8(info.accumulator_root_hash).expect("Getting accumulator root hash failed."),
            num_events,
    ...
    pub fn from_transaction(
        ...
        let txn_data = transaction.txn_data.expect("Txn Data doesn't exit!");
        let version = transaction.version as i64;
        let transaction_type = TransactionType::from_i32(transaction.r#type).expect("Transaction type doesn't exist!").as_str_name().to_string();
        let transaction_info = transaction.info.as_ref().expect("Transaction info doesn't exist!");
        let timestamp_in_seconds = transaction.timestamp.expect("Transaction timestamp doesn't exist!").seconds;
        match txn_data {
            TxnData::User(user_txn) =>  {
                let (user_txn_output, signatures) =
                    UserTransaction::from_transaction(&user_txn, timestamp_in_seconds, block_height, epoch, version);
                ...
                (
                    Self::from_transaction_info(
                        transaction_info,
                        Some(
                            serde_json::to_value(&user_txn.request.expect("Getting user request failed.").payload.expect("Getting payload failed."))
            ...
            TxnData::StateCheckpoint(state_checkpoint_txn) => (
                Self::from_transaction_info(
                    transaction_info,
                    None,
                    version,
