...
struct Entry<V: TransactionWrite> {
    ...
    /// and also for uniquely identifying associated executables. None implies
    /// that the stored module (WriteOp) actually corresponded to a deletion.
    maybe_hash: Option<HashValue>,
}
...
impl<V: TransactionWrite> Entry<V> {
    pub fn new_write_from(flag: usize, module: V) -> Entry<V> {
        let maybe_hash = module.extract_raw_bytes().map(|bytes| {
...
impl<K: Hash + Clone + Eq, V: TransactionWrite, X: Executable> VersionedCode<K, V, X> {
    ...
    fn read(
        ...
        key: &K,
        txn_idx: TxnIndex,
    ) -> anyhow::Result<(Arc<V>, Option<HashValue>), MVCodeError> {
        use MVCodeError::*;
        ...
        match self.values.get(key) {
            Some(v) => {
                if let Some((idx, entry)) = v.versioned_map.range(0..txn_idx).next_back() {
                    ...
                    Ok((entry.module.clone(), entry.maybe_hash))
                } else {
                    Err(NotFound)
                }
            },
    ...
    pub(crate) fn fetch_module(
        ...
        match self.read(key, txn_idx) {
            Ok((module, maybe_hash)) => {
                let v = self.values.get(key).expect("Path must exist");

                Ok(match maybe_hash {
                    Some(hash) => {
                        match v.executables.get(&hash) {
                            Some(x) => Executable((x.clone(), false)), // from the block
                            None => Module((module, maybe_hash)),
                        }
                    },
                    None => Module((module, None)), // Deletion
                })
            },
            Err(NotFound) => {
                match self.values.get(key) {
                    Some(v) => v
                        .base_executable
                        .as_ref()
                        .map(|x| Executable((x.clone(), true))) // storage-version
                        .ok_or(NotFound),
                    None => Err(NotFound),
                }
            },
