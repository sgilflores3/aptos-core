...
mod tests {
    ...
    async fn test_check_network_connectivity() {
        ...
        assert!(help_message.contains("USAGE:")); // We expect the command to return USAGE info
        ...
        assert!(error_message.contains("Invalid address")); // The error message should contain this

        // Verify that an invalid chain-name will return an error
        let args = &["aptos", "node", "check-network-connectivity", "--address", "/ip4/34.70.116.169/tcp/6182/noise-ik/0x249f3301db104705652e0a0c471b46d13172b2baf14e31f007413f3baee46b0c/handshake/0", "--chain-name", "invalid-chain"];
        let error_message = run_tool_with_args(args).await.unwrap_err();
        assert!(error_message.contains("Not a reserved chain name")); // The error message should contain this

        // Verify that a failure to connect will return a timeout
        let args = &["aptos", "node", "check-network-connectivity", "--address", "/ip4/31.71.116.169/tcp/0001/noise-ik/0x249f3301db104705652e0a0c471b46d13172b2baf14e31f007413f3baee46b0c/handshake/0", "--chain-name", "testnet"];
        let error_message = run_tool_with_args(args).await.unwrap_err();
        assert!(error_message.contains("Timed out while checking endpoint")); // The error message should contain this
    }
